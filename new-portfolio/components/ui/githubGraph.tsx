...
export function GitHubGraph() {
  ...
  const token = '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'
  ...
  const monthLabels = weeks.reduce(
    (acc: { month: string; offset: number }[], week, i) => {
      if (week && week[0]) {
  ...
  return (
    <div className="p-6 bg-background/80 rounded-lg">
      <h3 className="text-lg font-medium mb-8 text-center">GitHub Contributions</h3>
            ...
            <div className="flex gap-[3px]">
              {monthLabels.map(({ month, offset }, i) => (
                <div
                  ...
                  style={{
                    width: `${weeks[offset].length * 15}px`,
                    textAlign: "left",
                    minWidth: `${weeks[offset].length * 15}px`, // Ensure consistent width
                  }}
                >
                  {month}
                </div>
